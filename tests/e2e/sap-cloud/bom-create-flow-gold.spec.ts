 ...
 * MARKER: e2egold-bom-create-flow
...
test.describe('BOM Create V4 End-to-End Flow', () => {
  test('Complete BOM Create Flow - V4 MDC Single Session', async ({ page, ui5 }) => {
    ...
    await test.step('Step 2: Open Create BOM Dialog', async () => {
      ...
      // In V4 MDC apps, the DatePicker value is managed through MDC ConditionModel.
      // The inner control's getValue()/getProperty('value') returns empty through
      // the proxy because MDC doesn't bind the value directly to the inner control.
      // Verify the control exists and type is correct (proves field rendered).
      // The outer MDC Field's getConditions() confirms a date value is bound.
      const validFromInner = await ui5.control({ id: IDS.validFromInner });
      const validFromType = await validFromInner.getControlType();
      test.info().annotations.push({
        type: 'info',
        description: `Valid From field type: ${validFromType} (value managed by MDC ConditionModel)`,
      });
      // MDC Field wraps either sap.m.DatePicker or sap.ui.mdc.field.FieldInput
      expect(validFromType).toBeTruthy();

      // Verify the outer MDC Field has conditions set (proves date is bound)
      const validFromField = await ui5.control({ id: IDS.validFrom });
      const validFromConditions = (await validFromField.getProperty('conditions')) as unknown[];
      test.info().annotations.push({
        type: 'info',
        description: `Valid From conditions: ${JSON.stringify(validFromConditions)}`,
      });
      expect(validFromConditions).toBeTruthy();
    });
    ...
    await test.step('Step 6: Fill Form with Valid Data', async () => {
      ...
      // MDC FieldInput displays description text with key (e.g., "Civil Works (2)")
      // after binding resolves, NOT just the raw key. Use toContain for comparison.
      expect(finalVerification.materialValue).toContain(materialValue.material!);
      expect(finalVerification.plantValue).toContain(plantValue.plant!);
