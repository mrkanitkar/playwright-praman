...
1. **SAP UI5 Framework** — sap.m.*, sap.ui.core.*, sap.ui.table.*, sap.f.*, sap.uxap.*
...
These are the most-used controls in SAP Fiori apps. Each gets a typed proxy in `src/proxy/typed/`:
...
| No Web Component support | Use `WebComponentAdapter` (Shadow DOM locators) |
...
### 4.2 Authentication Strategy Interface
...
### 6.2 OData Module Pattern

```typescript
// src/modules/odata.ts — OData CRUD with optional Zod validation
    ...
    const raw = await page.evaluate(
      ({ model, entityPath }) => {
        return new Promise((resolve, reject) => {
          const oModel = sap.ui.getCore().getModel(model);
...
| `interact.ts` | Control interaction (fireEvent, press, setValue) | `bridge/browser-scripts/` |
...
## 8. Web Component Knowledge (Hybrid Adapter)
