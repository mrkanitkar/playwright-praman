...
Some files in this repo are generated by scripts. Understanding which files are generated,
by what, and when prevents confusion during development.

| File(s)                                  | Generator                       | Committed?    | When to regenerate                             |
| ---------------------------------------- | ------------------------------- | ------------- | ---------------------------------------------- |
| `src/core/types/controls.ts`             | `npm run generate:proxies`      | ✅ Yes        | After adding/removing UI5 control types        |
| `capabilities.yaml`, `capabilities.json` | `npm run generate:capabilities` | ✅ Yes        | After adding/changing `@capability` TSDoc tags |
| `llms.txt`, `llms-full.txt`              | `npm run generate:llms-txt`     | ✅ Yes        | After changing public API surface              |
| `src/version.ts`                         | tsup `define` at build time     | ✅ Yes        | Automatic during `npm run build`               |
| `skills/.../SKILL.md`                    | `npm run generate:skill-md`     | ❌ Gitignored | Local dev only                                 |
| `src/proxy/typed/*.generated.ts`         | `npm run generate:proxies`      | ❌ Gitignored | Local dev only                                 |
| `*.sbom.json`                            | `npm run generate:sbom`         | ❌ Gitignored | CI release only                                |

**Note:** `src/proxy/typed/` is expected to be empty at HEAD — the generated proxy files
are gitignored. This is intentional: the 199-control typed surface lives in the committed
`src/core/types/controls.ts`, not in per-control generated files.

The full build pipeline (`npm run build:full`) runs: `generate:capabilities` → `generate:skill-md` → `build` → `generate:llms-txt`.
