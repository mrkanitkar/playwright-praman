...
description: Step-by-step diagnostic guide for common Praman issues with error-code-driven flowchart
---

Step-by-step diagnostic guide for common Praman issues. For the full error code catalog, see the [Error Reference](./errors.md).
...
2. Refer to the [Configuration guide](./configuration.md) for all valid options.
...
When a test generated by `praman-sap-generate` or `praman-sap-heal` fails, follow these steps. For a comprehensive guide, see [Debugging Agent Failures](./debugging-agent-failures.md).
