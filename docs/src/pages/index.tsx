...
function AgenticSVG(): ReactNode {
  ...
  return (
    ...
    >
      ...
      <text x={hx} y={hy - 4} textAnchor="middle" fill="#1e3c72" fontSize="9" fontWeight="800">
        PRAMAN
      </text>
      <text x={hx} y={hy + 8} textAnchor="middle" fill="#1e3c72" fontSize="7.5" fontWeight="600">
        Plugin
      </text>
      {agents.map((a) => (
        <g key={`node-${a.label}`}>
...
function PlaywrightSVG(): ReactNode {
  ...
  return (
    ...
    >
      ...
      <text x="140" y="21" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="8.5">
        playwright-praman · test.ts
      </text>
...
function ArchitectureSVG(): ReactNode {
  const layers = [
    {
      label: 'AI & Intents',
      sub: 'capabilities · SKILL.md · intent APIs',
    ...
    {
      label: 'Control Proxy',
      sub: 'typed proxies · 7 return-type handlers',
      fill: 'rgba(255,255,255,0.09)',
      stroke: 'rgba(255,255,255,0.28)',
    },
    {
      label: 'Bridge & Browser',
      sub: 'UI5 bridge · browser scripts · TTL cache',
      fill: 'rgba(255,255,255,0.06)',
      stroke: 'rgba(255,255,255,0.22)',
    },
    {
      label: 'Core Infrastructure',
      sub: 'config · errors · logging · selectors',
        ...
        return (
          <g key={label}>
            <rect
              ...
              strokeWidth="1"
            />
            <text x="24" y={y + 12} fill="white" fontSize="9.5" fontWeight="700">
              {label}
            </text>
            <text x="24" y={y + 23} fill="rgba(255,255,255,0.52)" fontSize="7.5">
              {sub}
            </text>
            {i < 4 && (
              <text x="252" y={y + 23} fill="rgba(255,255,255,0.38)" fontSize="14">
                ↓
              </text>
            )}
          </g>
...
function EnterpriseSVG(): ReactNode {
  ...
  return (
    ...
    >
      <path
        d="M140 8 L202 36 L202 108 Q202 158 140 180 Q78 158 78 108 L78 36 Z"
        fill="rgba(255,255,255,0.06)"
        stroke="rgba(255,215,0,0.65)"
        strokeWidth="2"
      />
      <path
        d="M140 24 L190 48 L190 108 Q190 148 140 166 Q90 148 90 108 L90 48 Z"
      ...
      />
      <path
        d="M132 38 Q132 29 140 29 Q148 29 148 38"
        fill="none"
        stroke="rgba(255,215,0,0.7)"
        strokeWidth="2"
      />
      ...
      {checks.map(({ y, label }) => (
        <g key={label}>
          <circle
            ...
            strokeWidth="1"
          />
          <text x="110" y={y} textAnchor="middle" fill="#ffd700" fontSize="11" fontWeight="700">
            ✓
          </text>
          <text x="126" y={y} fill="rgba(255,255,255,0.88)" fontSize="10.5" fontWeight="500">
            {label}
          </text>
        </g>
      ))}
        ...
        return (
          <g key={role}>
            ...
            <circle
              ...
              strokeWidth="1"
            />
            <text x="32" y={y + 29} textAnchor="middle" fill="rgba(255,215,0,0.9)" fontSize="12">
              {icon}
            </text>
            {/* Role label */}
            <text x="56" y={y + 17} fill="white" fontSize="10" fontWeight="700">
              {role}
            </text>
            {/* Description — wrapped into two lines via tspan */}
            <text x="56" y={y + 30} fill="rgba(255,255,255,0.68)" fontSize="8.2" fontWeight="400">
              <tspan x="56" dy="0">
                {desc.length > 52 ? desc.slice(0, 52) : desc}
              </tspan>
              {desc.length > 52 && (
                <tspan x="56" dy="11">
                  {desc.slice(52)}
...
const SLIDES: CarouselSlide[] = [
  {
    ...
    subtitle:
      'End-to-end testing for SAP S/4HANA, Fiori Launchpad, and UI5 apps. Open source, flake-free, CI-ready.',
  ...
  },
  {
    ...
    subtitle:
      'Each role interacts with Praman through its own interface — no shared abstraction, no lowest-common-denominator API.',
  ...
  },
  {
    ...
    subtitle:
      'Extends @playwright/test with SAP UI5 locators, SAP login automation, and codegen. Parallel execution, auto-wait, trace viewer — unchanged.',
  ...
  },
  {
    ...
    subtitle:
      '6-layer architecture: Core → Bridge → Proxy → Fixtures → AI. 3 interaction strategies, TTL+LRU caching, 10 error subclasses, full OTel tracing.',
  ...
  },
  {
    ...
    title: 'Deploy with evidence. Not hope, not hope-ware.',
    subtitle:
      'OpenTelemetry tracing, compliance reporters, defect video, SBOM, CodeQL scanning. Free, open-source, CISO-approved — replace Tricentis and Tosca without the license bill.',
      ...
      <section className="praman-glance-section">
        ...
        <div className="praman-info-cards">
          ...
          <div className="praman-info-card">
            ...
            <p>
              Business analysts define the process. AI agents — Claude, Copilot, Jules — generate
              the tests. No scripting required. Agents connect to your live SAP system via{' '}
              <strong>Playwright MCP</strong> (JSON-RPC) or <strong>Playwright CLI</strong>{' '}
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        strokeWidth="1.5"
      />
      <text x="80" y="38" textAnchor="middle" fill="#22c55e" fontSize="9" fontWeight="700">
        ▶
      </text>
      {/* Arrow */}
      ...
      <text x="80" y="170" textAnchor="middle" fill="rgba(255,255,255,0.55)" fontSize="6.5">
        Material · Qty · Vendor · Plant
      </text>
      {/* Arrow */}
      ...
      {/* Gateway diamond */}
      <polygon
        points="80,191 108,205 80,219 52,205"
        fill="rgba(245,158,11,0.13)"
        stroke="rgba(245,158,11,0.5)"
        strokeWidth="1"
      />
      <text x="80" y="208" textAnchor="middle" fill="rgba(255,255,255,0.65)" fontSize="6.5">
        Approved?
      </text>
      {/* Arrow (Yes) */}
  ...
  return (
    <div className="praman-agent-flow-outer">
      <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
        ...
        <h2 className="praman-section-title">
      ...
      <p
        style={{
          ...
          margin: '0 auto 2rem',
          fontSize: '1.08rem',
          color: 'var(--praman-ink-secondary)',
          lineHeight: 1.7,
        }}
      >
        ...
        generate a structured test plan, and deliver production-ready Playwright scripts — covering
        SAP end-to-end quality from requirement to deployment evidence. No scripting. No selectors.
        No delays.
      </p>
      <div className="praman-agent-flow-grid">
        ...
        <div className="praman-agent-flow-panel">
          ...
          <div className="praman-code-block praman-flow-block">
            <div className="praman-code-dots">
              <span />
              <span />
              <span />
            </div>
          ...
          <div className="praman-code-block praman-flow-block">
            ...
            <pre>
              <code>
                ...
                {'\n'}
                {'1. '}
                <span className="fn">{'Open FLP → Purchasing tile'}</span>
                {'\n'}
                {'   '}
                ...
                {'\n'}
                {'1. '}
                <span className="fn">{'Set Vendor: 1000 (Müller GmbH)'}</span>
                {'\n'}
                {'   '}
                <span className="str">{'✓'}</span>
                {' expect: Vendor name auto-filled\n'}
                {'2. '}
                <span className="fn">{'Purch.Org: 1010, Group: 001'}</span>
                {'\n'}
                {'   '}
                <span className="str">{'✓'}</span>
                ...
                {'\n'}
                {'1. '}
                <span className="fn">{"Click 'Order' → Post PO"}</span>
                {'\n'}
                {'   '}
                <span className="str">{'✓'}</span>
                {' expect: PO# 4500000xxx\n'}
                {'2. '}
                <span className="fn">{'OData: GET /A_PurchaseOrder'}</span>
                {'\n'}
                {'   '}
                <span className="str">{'✓'}</span>
                {' expect: Status = Open'}
          ...
          <div className="praman-code-block praman-flow-block">
            ...
            <pre>
              <code>
                ...
                <span className="fn">test</span>
                {'('}
                <span className="str">{"'verify purchase order'"}</span>
                {','}
                {'\n'}
                {'  '}
                <span className="kw">async</span>
                {' ({ '}
                <span className="fn">ui5</span>
                {', '}
                <span className="fn">odata</span>
                {' }) => {'}
                {'\n'}
                {'\n'}
                {'  '}
                <span className="kw">await</span> <span className="fn">ui5</span>
                {'.'}
                <span className="fn">control</span>
                {'({'}
                {'\n'}
                {'    id: '}
                <span className="str">{"'vendorInput'"}</span>
                {','}
                {'\n'}
                {'    '}
                ...
                {'\n'}
                {'  '}
                <span className="kw">await</span> <span className="fn">odata</span>
                {'.'}
                ...
                {'\n'}
                {'    .'}
                <span className="fn">toHaveValue</span>
                {'('}
      ...
      <div className="praman-reports-grid">
        ...
        <div className="praman-report-card praman-report-card--blue">
          ...
          <p className="praman-report-card-desc">
            Each stakeholder sees only what matters to their role — generated from the same test
            run.
          </p>
        ...
        <div className="praman-report-card praman-report-card--purple">
          ...
          <p className="praman-report-card-desc" style={{ marginBottom: '0.2rem' }}>
            Asset Accounting{' '}
            <span style={{ color: 'var(--capgemini-blue)', fontWeight: 700 }}>(FI-AA)</span>
          </p>
          <div className="praman-report-metrics">
            ...
            <div className="praman-report-metric">
              <span className="praman-report-metric-val" style={{ color: '#f59e0b' }}>
                1,234
              </span>
        ...
        <div className="praman-report-card praman-report-card--dark">
          ...
          <p className="praman-report-card-desc">
            Identifies bottlenecks by business flow — pinpoint which process is slowing your
            ...
            {heatmapRows.map(({ label, time, pct, status, color }) => (
              <div key={label} className="praman-heatmap-row">
                ...
                <div className="praman-heatmap-bar-track">
                  <div
                    className="praman-heatmap-bar-fill"
                    style={{ width: `${pct}%`, background: color }}
                  />
                </div>
                <span className="praman-heatmap-status" style={{ color }}>
                  {status}
                </span>
              </div>
            ))}
        ...
        <div className="praman-report-card praman-report-card--green">
          ...
          <ul className="praman-report-evidence-list">
            <li>
              <span className="praman-evidence-check">&#10003;</span> OData response snapshots per
              transaction
            </li>
            <li>
              <span className="praman-evidence-check">&#10003;</span> Defect video with step-level
              timestamps
            </li>
            <li>
              <span className="praman-evidence-check">&#10003;</span> Accessibility score (WCAG 2.1)
            </li>
            <li>
              <span className="praman-evidence-check">&#10003;</span> Data integrity diff: ECC vs
              S/4HANA
            </li>
            <li>
              <span className="praman-evidence-check">&#10003;</span> CISO-approved, OpenTelemetry
              traced
            </li>
          </ul>
        </div>
    ...
    <section style={{ padding: '3rem 2rem 5rem', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div className="praman-pitch-grid">
        ...
        <div className="praman-pitch-cell">
          ...
          <p>
            Submit a business process or test case — Praman&apos;s AI agents autonomously generate
            test plans, produce Playwright scripts, validate OData integrity, and interact with
            every UI5 control. From requirement to evidence in minutes, not sprints.
          </p>
        </div>
        <div className="praman-pitch-cell">
          ...
          <p>
            Data Integrity Validation, Business Flow Performance Heatmap in every run, Accessibility
            Testing, Compliance.
          </p>
        </div>
        <div className="praman-pitch-cell">
...
function Philosophy(): ReactNode {
  return (
    <div className="praman-philosophy">
      <section
        style={{ padding: '5rem 2rem', maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}
      >
        ...
        <div className="praman-quote">
          &ldquo;In Ancient India, <em>Pram&#x101;&#x1E47;a</em> was the standard to separate fact
          from illusion. In the Modern Enterprise, <strong>Praman</strong> separates a
          &lsquo;Successful Deployment&rsquo; from a &lsquo;Business Disruption.&rsquo;&rdquo;
        </div>
  ...
  return (
    <section style={{ textAlign: 'center', padding: '2.5rem 2rem' }}>
      <div className="praman-assurance">
        <div className="praman-assurance-item">
          <span>&#10003;</span> CISO-Approved
        </div>
        <div className="praman-assurance-item">
  ...
  return (
    <section className="praman-cta" style={{ textAlign: 'center', padding: '5rem 2rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
        <Link className="praman-btn praman-btn-dark" to="/docs">
          Get Started
        </Link>
        <Link
          className="praman-btn praman-btn-light"
