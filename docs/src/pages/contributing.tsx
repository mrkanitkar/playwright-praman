    ...
    <Layout title="Contributing" description="Contributing to Praman">
      <main className="praman-prose-page">
        ...
        <Section title="Development Setup">
          <h3>Prerequisites</h3>
          <ul>
            <li>Node.js 20+ (see <code className="praman-prose-code">.nvmrc</code> for exact version)</li>
          ...
          <table className="praman-prose-table">
            ...
            <tbody>
              {[
                ...
                <tr key={cmd}>
                  <td><code className="praman-prose-code">{cmd}</code></td>
                  <td>{purpose}</td>
                </tr>
              ))}
            </tbody>
...
Layer 1: Core          — Logging, config, errors, utilities`}</pre>
          ...
          <p><strong>Sub-path exports:</strong>{' '}
            <code className="praman-prose-code">.</code>,{' '}
          ...
          <p><strong>Path aliases:</strong>{' '}
            <code className="praman-prose-code">#core/*</code>,{' '}
        ...
        <Section title="Code Standards">
          <p>TypeScript strict mode. No <code className="praman-prose-code">any</code>, no <code className="praman-prose-code">{'as unknown as T'}</code>. ESM only. Node builtins use <code className="praman-prose-code">node:</code> prefix. Relative imports include <code className="praman-prose-code">.js</code> extension. Maximum 300 LOC per file.</p>
          <p><strong>Documentation:</strong> Microsoft TSDoc (not JSDoc). Every public function must have TSDoc with an <code className="praman-prose-code">@example</code> tag.</p>
          <p><strong>Errors:</strong> All errors extend <code className="praman-prose-code">PramanError</code> with <code className="praman-prose-code">code</code>, <code className="praman-prose-code">attempted</code>, <code className="praman-prose-code">retryable</code>, <code className="praman-prose-code">suggestions[]</code>. Use pino logger — never <code className="praman-prose-code">console.log</code>.</p>
...
test(core): add config validation edge cases`}</pre>
        ...
        <Section title="Testing">
          <p><strong>Unit:</strong> Vitest 4.x with <code className="praman-prose-code">describe</code> / <code className="praman-prose-code">it</code> pattern. File naming: <code className="praman-prose-code">*.test.ts</code>. Hermetic — no network calls.</p>
          <p><strong>Integration:</strong> Playwright with <code className="praman-prose-code">test.step()</code>. File naming: <code className="praman-prose-code">*.spec.ts</code>. Requires SAP credentials.</p>
          <p><strong>Coverage:</strong> Tier 1 (errors): 100%. Tier 2 (core): 95%. Tier 3 (global): 90%. Per-file enforcement.</p>
        </Section>

        <Section title="Git Hooks">
          <table className="praman-prose-table">
            <thead><tr><th>Hook</th><th>Action</th></tr></thead>
            <tbody>
              <tr><td><strong>pre-commit</strong></td><td>lint-staged (ESLint + Prettier on staged files)</td></tr>
              <tr><td><strong>commit-msg</strong></td><td>commitlint validation</td></tr>
              <tr><td><strong>pre-push</strong></td><td>typecheck + unit tests with coverage + build</td></tr>
            </tbody>
          </table>
        </Section>
        ...
        <Section title="Pull Requests">
          <ol>
            <li>Create a feature branch from <code className="praman-prose-code">main</code></li>
            <li>Make your changes following the code standards above</li>
            <li>Ensure <code className="praman-prose-code">npm run ci</code> passes locally</li>
