...
**[playwright-praman](https://praman.dev)** is a Playwright plugin with typed fixtures, a bridge layer for UI5 control access, OData APIs, Fiori Elements page objects, BTP WorkZone support, and an agentic AI workflow (Plan → Generate → Heal) with 9 agents in 3 variants (MCP browser, CLI token-efficient, vanilla Playwright) that lets teams go from a live SAP app to passing tests with minimal hand-coding. Its CLI provides an offline capability manifest, a spec compliance validator, and pre-built discovery scripts that work via Playwright CLI. It has the deepest SAP domain coverage of the four.
...
| Agent workflows (Plan → Generate → Heal)           |       ✗        |       ✗        |                           ✗                            |             ✓ 9 Claude Code agents (3 variants × 3 roles)             |
...
| Debug CLI (attach + step-through)             |             ✗              |                      ✗                       |  ✗   |                        ✓ --debug=cli + playwright-cli attach                        |
