---
title: 'Prompt: Create Sales Orders — End-to-End SAP Automation'
authors: [maheshwar]
tags: [example-prompts, prompts, sales-order, delivery, billing, sap-ui5, ai, playwright]
description: >
  A production-ready AI prompt for automating the complete SAP Sales Order lifecycle
  (VA01, VL01N, VL06O, VL03N, F2875, F0798, F0797) using Praman and Playwright.
keywords:
  - praman
  - playwright
  - sap
...
# Create Sales Orders — End-to-End SAP Automation Prompt

A structured, prompt-engineered template for AI agents to automate
the complete SAP Sales Order lifecycle — from order creation through
...
Find the prompt in `praman-prompts/create-sales-orders.prompt.md` in your project.
...
<details>
<summary>Click to expand the full prompt (396 lines)</summary>
...
- **Two-Phase Wizard** — Phase 1 asks questions one-by-one to confirm test data before Phase 2 executes
- **NEVER assume** — all values, control IDs, and app behavior must be confirmed by the user or discovered via `browser_run_code`
- **Fixture-only pattern** — Praman fixtures for ALL UI5 elements, never raw `page.click()`
- **App-type-aware selectors** — Fiori Elements stable IDs for F2875/F0798/F0797; runtime discovery via `browser_run_code` for VA01/VL01N/VL06O/VL03N (ABAP freestyle)
- **Input triple** — `setValue()` + `fireChange()` + `waitForUI5()` for every input field
- **Dialog awareness** — `searchOpenDialogs: true` for controls inside dialogs
- **No DOM selectors** — CSS, XPath, and record-playback selectors are forbidden
- **Semantic naming** — use business names (SoldToParty, TotalAmount) not technical IDs
- **Backend OData verification** — every Save/Create/Post/Finalize followed by `verify-backend` OData call
- **Self-check checklist** — 11-point validation before submitting output

## Known Limitations

- VA01, VL01N, VL06O, and VL03N are ABAP freestyle apps (not Fiori Elements) — their control IDs must be discovered at runtime via `browser_run_code`, not assumed from annotation patterns.
- Step 3 is absent from the source test data — verify with the business user whether this is intentional.
- Two SAP roles are required (Internal Sales Representative and Shipping Specialist), requiring a re-login at step 24.
- Batch split handling at step 16-17 has conditional logic that depends on the specific material and plant configuration.
- Optional steps 45-47 (cancel billing, update attachments, update texts) may not be applicable to all billing document types.
